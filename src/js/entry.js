...
AudioManager.prototype = {
  ...
  changeCurrentTime: function(ratio) {
...
let controlManager = (function () {
  let $playBtn = $scope.find('.play-btn');
  ...
  function setProgress() {
    let frame = function () {
      ...
      $curTime.text(time);
      if (ratio <= 1 && $playBtn.hasClass('playing')) {
  ...
  function bindProgressEvent() {
    ...
    $slidePoint.on('touchstart', function (e) {
      ...
      let translatePercent = (ratio - 1) * 100;
      audioManager.changeCurrentTime(ratio);
      if ($playBtn.hasClass('playing')) {
        audioManager.play();
      } else {
        setProtopTranslate(translatePercent);
